...
sum_data <- read.csv("datasets/duration_aridity_texture_irrigation.csv")

datasettemp <- split(sum_data, sum_data$treatment=="short term")
short <- datasettemp$'TRUE'
datasettemp <- split(sum_data, sum_data$treatment=="medium term")
medium <- datasettemp$'TRUE'
datasettemp <- split(sum_data, sum_data$treatment=="long term")
long <- datasettemp$'TRUE'

# Set TIFF output parameters
tiff("duration_aridity_texture_irrigation.tiff", width = 9, height = 4, units = 'in', res = 300)
...
axis (side = 2, at = short$ID, labels = paste(short$SubGroup)
      , cex=1
      , las = 2)

text(ifelse(short$High_Perc > 50, short$Low_Perc-10, short$High_Perc+7), short$ID
     , paste(short$n_study), cex=0.85, adj=0)

# Separation lines
abline(h=c(2,6,10), col="black", lty=3, lwd=2)
...
text(ifelse(long$High_Perc > 62, long$Low_Perc-10, long$High_Perc+14), long$ID
     , paste(long$n_study), cex=0.85, adj=0)
