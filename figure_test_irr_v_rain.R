...
sum_data <- read.csv("datasets/irr_v_rain_aridity_texture_irrigation_depth.csv")
...
tiff("Figure test - irr v rain aridity.tiff", width = 4, height = 7, units = 'in', res = 300)
...
axis (side = 1, at = seq(-30, x_max, 10), labels = seq(-30, x_max, 10)
      , cex=0.75
      , las = 1)
...
text(ifelse(sum_data$High_Perc > 50, sum_data$Low_Perc-10, sum_data$High_Perc+5), sum_data$ID
...
mtext("Irrigated vs. Rainfed Agriculture", side=3, cex=0.75)
