...
sum_data$irrig_water_type[sum_data$irrig_water_type == "sewage"] <- "waste"
sum_data$irrig_water_type[sum_data$irrig_water_type == "mixed"] <- "waste"

temp_dataset <- split(sum_data, sum_data$irrig_water_type=="waste")
sum_data <- temp_dataset$'FALSE'
...
tiff("Figure 6 - initial_OC_per_change_all Aug 31.tiff", width = 5, height = 3, units = 'in', res = 300)
...
p + theme_bw() + theme(legend.position = "top", legend.title = element_blank(),
                        ...
                        axis.title.y = element_text(size = 8)) + annotate("text", x = 50, y = 45, label = "y = 15.37 - 0.29x", size=2)
