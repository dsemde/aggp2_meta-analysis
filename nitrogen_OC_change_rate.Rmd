---
title: "OC by Total Nitrogen at T1"
output:
  html_document:
    df_print: paged
---
...
```{r}
ggplot(d10, aes(x=total_N_t1, y=OC_change_rate)) + geom_point() + geom_smooth(method=lm)
```
...
```{r}
ggplot(d20, aes(x=total_N_t1, y=OC_change_rate)) + geom_point() + geom_smooth(method=lm)
```
...
```{r}
ggplot(d30, aes(x=total_N_t1, y=OC_change_rate)) + geom_point() + geom_smooth(method=lm)
```
...
```{r}
ggplot(d30p, aes(x=total_N_t1, y=OC_change_rate)) + geom_point() + geom_smooth(method=lm)
```
...
```{r}
ggplot(d10, aes(x=total_N_t1, y=per_OC_change)) + geom_point() + geom_smooth(method=lm)
```
...
```{r}
ggplot(d20, aes(x=total_N_t1, y=per_OC_change)) + geom_point() + geom_smooth(method=lm)
```
...
```{r}
ggplot(d30, aes(x=total_N_t1, y=per_OC_change)) + geom_point() + geom_smooth(method=lm)
```
